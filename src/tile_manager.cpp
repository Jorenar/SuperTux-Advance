    ...
    freelist(refcount), tileset(0)
{
    ...
    for (uint8_t rom_id = 0; rom_id < 95; ++rom_id) {
        for (int i = 0; i < 32; ++i) {
            ((uint16_t*)CHAR_BASE_ADR(0))[(MAX_VRAM_ID+rom_id)*32 + i] = ((uint16_t*)fontTiles)[rom_id*32 + i];
        }
    }
...
{
    const uint16_t* ptr = (const uint16_t*)(antarcticaTiles); // tileset->get_tile(rom_id);
    for (int i = 0; i < 32; ++i) {
